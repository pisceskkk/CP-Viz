...
### Computing `remote_target_ranks` (vectorized)

Both directions share the same two-step structure:
1. **Head dimension** – compute the valid TP rank range via KV group overlap.
2. **Token dimension** – solve the linear congruence `flat_idx % g == target_mod`
   for the strided set of valid `pcp_rank` values.

The key insight is that for a fixed `tp_rank`, the valid `pcp_rank` values satisfying
`flat_idx % g == target_mod` form an arithmetic sequence with common difference `g`,
which can be generated with a single `arange`.

```python
import numpy as np
from math import gcd

def get_prefill_ranks_for_decode_rank(
    dcp_local_rank_D: int,
    tp_rank_D: int,
    *,
    tp_size_D: int,
    dcp_size_D: int,
    ...
    """Return prefill rank indices whose KV overlaps with the given decode rank."""
    # ── Head dimension ──────────────────────────────────────────────────────────
    eff_tp_P   = min(tp_size_P, H_kv)
    eff_tp_D   = min(tp_size_D, H_kv)
    kv_grp_D   = tp_rank_D * eff_tp_D // tp_size_D
    kv_lo      = kv_grp_D * eff_tp_P // eff_tp_D
    kv_hi      = ((kv_grp_D + 1) * eff_tp_P - 1) // eff_tp_D   # inclusive
    replicas_P = tp_size_P // eff_tp_P                           # KV replicas per group
    tp_P = np.arange(
        kv_lo * replicas_P, (kv_hi + 1) * replicas_P, dtype=np.int32
    )  # shape (T,)

    # ── Token dimension ─────────────────────────────────────────────────────────
    # flat_idx := tp_rank_P * pcp_size_P + pcp_rank_P
    # Overlap:  flat_idx % g == target_mod   (since g | dcp_size_P)
    # => for each tp_rank_P, valid pcp_rank_P values are an arithmetic sequence
    #    with first term `base` and common difference `g`
    g          = gcd(dcp_size_P, dcp_size_D)
    target_mod = int(dcp_local_rank_D % g)
    p_mod_g    = int(pcp_size_P % g)

    bases   = (target_mod - tp_P.astype(np.int64) * p_mod_g) % g  # (T,)
    S       = (pcp_size_P + g - 1) // g                            # max stride count
    strides = np.arange(S, dtype=np.int32) * g                     # (S,)

    pcp_cands = (bases[:, None] + strides[None, :]).astype(np.int32)  # (T, S)
    valid     = pcp_cands < pcp_size_P

    # ── Assemble rank indices ───────────────────────────────────────────────────
    # r_P = pcp_rank_P * tp_size_P + tp_rank_P
    return (pcp_cands * tp_size_P + tp_P[:, None])[valid]


def get_decode_ranks_for_prefill_rank(
    r_P: int,
    *,
    tp_size_P: int,
    dcp_size_P: int,
    ...
    """Return decode rank indices that require KV data from the given prefill rank."""
    tp_rank_P   = r_P % tp_size_P
    pcp_rank_P  = r_P // tp_size_P
    dcp_local_P = (tp_rank_P * pcp_size_P + pcp_rank_P) % dcp_size_P

    # ── Head dimension ──────────────────────────────────────────────────────────
    eff_tp_P   = min(tp_size_P, H_kv)
    eff_tp_D   = min(tp_size_D, H_kv)
    kv_grp_P   = tp_rank_P * eff_tp_P // tp_size_P
    kv_lo_D    = kv_grp_P * eff_tp_D // eff_tp_P
    kv_hi_D    = ((kv_grp_P + 1) * eff_tp_D - 1) // eff_tp_P
    replicas_D = tp_size_D // eff_tp_D
    tp_D = np.arange(
        kv_lo_D * replicas_D, (kv_hi_D + 1) * replicas_D, dtype=np.int32
    )  # shape (T,)

    # ── Token dimension ─────────────────────────────────────────────────────────
    g           = gcd(dcp_size_P, dcp_size_D)
    target_mod  = int(dcp_local_P % g)
    p_mod_g_D   = int(pcp_size_D % g)

    bases   = (target_mod - tp_D.astype(np.int64) * p_mod_g_D) % g  # (T,)
    S       = (pcp_size_D + g - 1) // g
    strides = np.arange(S, dtype=np.int32) * g

    pcp_cands = (bases[:, None] + strides[None, :]).astype(np.int32)  # (T, S)
    valid     = pcp_cands < pcp_size_D

    # r_D = pcp_rank_D * tp_size_D + tp_rank_D
    return (pcp_cands * tp_size_D + tp_D[:, None])[valid]
```

**Complexity:** `O(T × S)` NumPy operations, where `T` is the number of overlapping
TP ranks (usually 1 unless KV head counts differ) and `S = ⌈pcp_size / g⌉` (shrinks
as `g = gcd(dcp_size_P, dcp_size_D)` grows).

| Scenario | g | S | T | Result size |
|---|---|---|---|---|
| Same config on both sides | `dcp_size` | 1 | 1 | 1 (1:1) |
| P finer than D (`dcp_P > dcp_D`) | `dcp_D` | `pcp_P / dcp_D` | 1 | 1 |
| D finer than P (`dcp_D > dcp_P`) | `dcp_P` | 1 | 1 | 1 |
| GQA KV replication (`tp ≥ H_kv`) | any | S | `replicas` | `S × replicas` |
